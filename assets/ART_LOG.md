...
## assets/masters/title-bg.png → public/assets/sprites/title-bg.png — 2026-07-20

- **Purpose:** Opaque, full-bleed title-screen hero establishing shot. Empty foreground cornice is reserved for the runtime pigeon overlay; the broad center-left sky is reserved for the runtime title logotype.
- **References inspected:** images/normal.png and images/pleased.png at original resolution as style-only references for thick clean dark outlines, painterly cel shading, warm slightly desaturated color, and cozy comic atmosphere; their pigeon subject was explicitly excluded. assets/masters/bg-buildings-sheet.png and assets/masters/bg-buildings-sheet-2.png at original resolution as style/architectural-language references for masonry, windows, rooftop objects, line weight, and detail density; their magenta sprite-sheet backgrounds and incidental subjects were explicitly excluded.
- **Tool:** Codex built-in image_gen (model name not surfaced). One source generation was accepted. The untouched generator output was copied byte-for-byte to assets/masters/title-bg.png; master SHA-256 fc081d6d126c99d5672f05952724c7b95d419a3ebad5e4c21e74ab6084f7537c.
- **Dimensions:** master 1930×815, opaque sRGB true-color PNG; shipped 2560×1080, opaque sRGB indexed PNG with 256 colors. Shipped SHA-256 07f3d1cee91fe848946c0b793cdf96b12e697295a9eb99eff879262e989754c1.
- **Post-processing:** Preserved the generated source unchanged at assets/masters/title-bg.png. Created a separate true-color shipping candidate with ImageMagick 6: convert assets/masters/title-bg.png -auto-orient -resize '2560x1080^' -gravity center -extent 2560x1080 -alpha off -colorspace sRGB -strip /tmp/title-bg-truecolor.png. This used a center-weighted fill resize with a negligible vertical crop to match the exact 64:27 frame. Copied that candidate to public/assets/sprites/title-bg.png, then ran pngquant --force --skip-if-larger --output public/assets/sprites/title-bg.png 256 public/assets/sprites/title-bg.png. Compression reduced the shipping file from 2,815,809 bytes to 1,098,782 bytes.
- **QA:** Visually inspected the untouched master and final shipped image at original resolution, plus matching 1100×620 center-left sky crops from the pre-quantized and quantized exports. No pigeon, bird, person, vehicle, bicycle, aircraft, text, logo, watermark, border, or vignette is visible. The foreground stone/brick cornice is empty. The center-left contains a broad, low-detail dusty mauve-to-orange sky field; only small subdued clouds near its edges and low skyline details are present, so it is clear enough for a high-contrast title overlay. pngquant introduced no visible contouring/banding in the smooth gradient; subtle dithering was acceptable, so the smaller indexed PNG was kept. Final identify QA: 2560×1080, opaque=true, channels=srgb, type=Palette, colors=256.
- **Exact source-generation prompt:**
  > Use case: stylized-concept
  > Asset type: opaque full-bleed title-screen hero background for a cartoon game
  >
  > REFERENCE ROLES: Inspect all four supplied references before generating. images/normal.png and images/pleased.png are STYLE-ONLY references for thick clean dark outlines, painterly cel shading, warm slightly desaturated color, and cozy comic-book atmosphere. Do not depict, imply, silhouette, or hide the pigeon character anywhere. assets/masters/bg-buildings-sheet.png and assets/masters/bg-buildings-sheet-2.png are STYLE AND ARCHITECTURAL-LANGUAGE references for masonry, windows, rooftop objects, line weight, and detail density. Do not reproduce their magenta chroma-key fields, isolated sprite-sheet layout, pigeons, bicycle, signage, or other subjects. This new asset has no characters in it.
  >
  > PRIMARY REQUEST: Generate ONE new background art asset for this game's TITLE SCREEN — a hero establishing shot of the city, no pigeon, no people, no vehicles, no text/logo baked in (all of that is added separately in code as overlays).
  >
  > SUBJECT AND COMPOSITION: An ultra-wide cinematic view across a dense city skyline at golden-hour dusk, framed as if looking out from rooftop height — the kind of vantage a bird sitting on a cornice would have. Rooftops recede into the distance with varied silhouette shapes: water towers on stilts, brick chimneys, TV antennas, AC/stairwell boxes, a fire escape or two, a couple of taller apartment blocks toward the back. Sky: deep dusty blue at the very top, softening through dusty mauve/pink through the middle, warming into rich orange and amber near the horizon, with a big warm low sun (partially behind a rooftop silhouette is fine) and soft glow around it. A handful of distant windows already glowing warm as the light fades. In the immediate foreground along the bottom edge, an empty stone/brick rooftop cornice or ledge in silhouette (no bird perched on it — leave it empty, a pigeon overlay sprite will be composited on top of it afterward by the game code). Keep a clear, uncluttered band of sky roughly in the center-left area — that's where the game's title logotype will be overlaid in code, so don't place any busy detail, birds, or bright highlights there.
  >
  > PALETTE AND RENDERING: match the referenced images' established style exactly — cartoon game art, thick clean dark outlines on major shapes, painterly cel shading with soft gradients in the sky, warm slightly desaturated color palette, cozy comic-book atmosphere, absurdist-but-heartwarming tone. Not photorealistic, but noticeably more detailed/atmospheric/"hero shot" than a flat mobile-game background — this is the game's key art.
  >
  > FRAMING: ultra-wide 21:9-ish landscape aspect. Compose for a center-weighted crop to exactly 2560x1080 if the native canvas is less wide. Preserve the full skyline concept and the empty center-left text-safe sky through that crop. Content reaches naturally to all four edges. This is an opaque full-bleed background image, not a sprite: no transparency and no magenta chroma key.
  >
  > STRICT AVOID LIST: No text, no letters, no numbers, no logos, no signs, no watermark, no frame, no border, no vignette, no pigeon, no birds of any kind, no people, no human silhouettes, no vehicles, no bicycles, no aircraft, no characters or creatures. No bright highlights or busy rooftop detail inside the clear center-left title-safe sky band.
  >
  > OUTPUT INTENT: Preserve the untouched generated master as assets/masters/title-bg.png, then derive an opaque 2560x1080 shipping PNG at public/assets/sprites/title-bg.png.

## assets/masters/pigeon-perch-sheet.png → public/assets/sprites/pigeon-perch-f{0,1,2}.png — 2026-07-20

- **Purpose/state:** Static title-screen rooftop pigeon whose body stays perched while only its head turns: f0 = calm profile/looking away, f1 = half-lidded mid-turn, f2 = camera-facing conspiratorial smirk.
- **Tool and generation history:** Codex built-in `imagegen`; model identifier was not exposed. Two renders were made. The first otherwise-successful render was rejected after exact third-crops showed content touching/crossing nominal panel boundaries (`keyed-1` reached x=590 and `keyed-2` began at x=0). A targeted second edit/refinement reduced and re-centered the subjects; its generator output was accepted and copied byte-for-byte, untouched, to `assets/masters/pigeon-perch-sheet.png`. Temporary keyed validation files from the rejected render were not shipped.
- **References viewed before generation:** `images/normal.png`, `images/pleased.png`, `assets/masters/pigeon-flight-sheet.png`, `assets/masters/pigeon-look-sheet.png`, and `public/assets/sprites/title-bg.png`, all at original resolution. The title background was style/composition-only and was explicitly forbidden from appearing in the sprite. The accepted refinement attached the first candidate as edit target plus `assets/masters/pigeon-look-sheet.png`, `images/normal.png`, `images/pleased.png`, and `assets/masters/pigeon-flight-sheet.png`.
- **Identity invariants:** cartoon; thick clean dark outlines; painterly cel shading; slate-blue pigeon; orange eyes; purple eye patches; scruffy black brows; iridescent green-to-purple neck feathers; raunchy/absurdist tone. Standing/perched pose only, with folded wings, planted pink feet, and relaxed tail; never a flight silhouette.
- **Master and panel geometry:** untouched accepted master is 1774×887 sRGB PNG. Processing used three exact 591×887 crops at x=0, 591, and 1182; the generator's extra background-only column at x=1773 was omitted. Accepted keyed alpha bboxes before normalization were f0 `(79,187)-(536,715)`, f1 `(45,200)-(498,714)`, and f2 `(6,202)-(443,714)`.
- **Post-processing actually performed:** (1) crop each accepted panel with `convert ... -crop '591x887+<x>+0' +repage`; (2) key each crop exactly as requested with `convert in.png -fuzz 10% -transparent '#ff00ff' -channel A -morphology Erode Disk:1 +channel out.png`; (3) preserve each on a common 591×887 transparent canvas and translate the whole keyed frames horizontally by f0 `-35 px`, f1 `0 px`, f2 `+41 px` (ImageMagick `-roll`; no opaque pixels were near a wrap boundary); (4) resize every common canvas to exactly 296×444 with `-resize 296x444!`; (5) compress only shipping copies with `pngquant --force --skip-if-larger --output <ship.png> 256 <processed.png>`; (6) composite each shipped cutout over solid `#3a405a` with ImageMagick for visual QA at `assets/masters/pigeon-perch-f{0,1,2}-qa.png`.
- **Normalization/anchor measurements:** source lower-body centroids over y=350..640 after the horizontal translations are x=308.69, 308.84, and 308.80 (maximum drift 0.15 source px). At shipping resolution, body/feet is defined for QA as alpha >8 in the shared lower-body band y≥175. Its bboxes `(left,top,right,bottom)` are f0 `(21,175,244,358)`, f1 `(22,175,243,358)`, and f2 `(23,175,243,358)`. Corresponding body/feet centroids are f0 `(149.83,265.94)`, f1 `(149.96,266.30)`, f2 `(149.97,266.35)`: maximum centroid drift is 0.14 px horizontally and 0.41 px vertically. All three foot baselines are exactly y=358, so ground-line drift is 0 px. The 1–2 px bbox-edge variation is local silhouette/shading variation, not anchor translation or scale change.
- **Shipped outputs:** `public/assets/sprites/pigeon-perch-f0.png`, `public/assets/sprites/pigeon-perch-f1.png`, and `public/assets/sprites/pigeon-perch-f2.png`; all are 296×444 indexed sRGB PNGs with alpha.
- **Transparency QA:** corner alpha in TL/TR/BL/BR order is `[0,0,0,0]` for every shipped frame. Near-magenta pixels (`R>=224, G<=40, B>=224`) with nonzero alpha: f0=0, f1=0, f2=0. Opaque-content bboxes at alpha >8 are f0 `(21,93,251,358)`, f1 `(22,99,250,358)`, and f2 `(23,100,243,358)`. The master, all three shipped cutouts, and all three slate composites were visually inspected at original resolution; no clipped outline, magenta fringe, background/ledge, panel divider, or flight-pose artifact is visible.
- **Exact user prompt (verbatim):**
  > First view these reference images with your image viewing tool, in this repo: images/normal.png, images/pleased.png, assets/masters/pigeon-flight-sheet.png, assets/masters/pigeon-look-sheet.png, public/assets/sprites/title-bg.png. Study the character identity and the established "conspiratorial glance" expression before generating.
  >
  > CONTEXT: public/assets/sprites/title-bg.png is the game's title-screen background — a city skyline at dusk with an empty stone/brick rooftop cornice/ledge running along the bottom foreground. It is a STYLE AND COMPOSITION reference only — do not repaint it, do not include any part of it in your output, do not add any background at all to the new asset.
  >
  > TASK: Generate ONE new pigeon sprite sheet — a STANDING/PERCHED pose (NOT flying), for the title screen. Unlike every previous pigeon asset (which are all mid-flight with spread wings), this pigeon is standing still on the rooftop ledge with its wings folded at rest, like a real park pigeon minding its own business. It needs to read as a completely static, non-animated illustration at first glance, then reveal a head-turn.
  >
  > SUBJECT AND STATE: The same character — slate-blue pigeon, orange eyes, purple eye patches, scruffy black brows, iridescent green-to-purple neck feathers, thick clean dark outlines, painterly cel shading — but now standing upright on a ledge in profile-ish 3/4 side view, feet gripping the edge, wings fully folded/tucked against the body (no spread feathers, no flight silhouette), tail down and relaxed, chest slightly puffed, a calm/idle "just people-watching" stance.
  >
  > COMPOSITION: wide landscape sheet, exactly THREE equal side-by-side panels, no divider lines. The SAME standing pigeon, IDENTICAL body pose, size, silhouette, feet placement, wing-fold, tail, and implied ground line in all three panels — this is critical, since the game will swap between these three textures in place to animate ONLY the head turning, so the body must not shift by even a pixel of scale or position. ONLY the head/neck orientation and facial expression change, panel to panel:
  > - Panel 1 (left): head turned away, in calm profile or looking slightly off to the side/down at the city below, oblivious, at rest. This is the "just a static painting" pose.
  > - Panel 2 (middle): head mid-turn, rotating toward the viewer, eyes just starting to dart into view — a transitional in-between pose.
  > - Panel 3 (right): head fully turned, facing the camera dead-on, the same smug/conspiratorial expression as assets/masters/pigeon-look-sheet.png — both eyes half-lidded, sly knowing grin in the beak, one scruffy black brow raised, purple eye patches clearly visible, as if it just noticed the viewer watching it and is letting them in on a secret.
  >
  > PALETTE AND RENDERING: cartoon game art, thick clean dark outlines of consistent weight, painterly cel shading, warm slightly desaturated color palette, matching the referenced images exactly — slate-blue body, darker blue wing feathers, vivid orange irises, purple eye patches, scruffy black brows, gray beak, pink feet, iridescent green-to-purple neck feathers.
  >
  > BACKGROUND/CUTOUT: entire background must be solid flat pure magenta #ff00ff, no gradients, no ground/ledge drawn in, no shadows, no panel divider lines, nothing touching the image edges — just the three standing pigeons evenly spaced on flat magenta, feet near (but not touching) the bottom of each panel, ready to be composited onto the title screen's ledge by the game code.
  >
  > POST-PROCESSING (do this yourself with the available tools):
  > 1. Save the untouched generated master to assets/masters/pigeon-perch-sheet.png.
  > 2. Crop the three panels, then key each with `convert in.png -fuzz 10% -transparent '#ff00ff' -channel A -morphology Erode Disk:1 +channel out.png`.
  > 3. Normalize all three panels onto a common padded canvas so the shared body/feet/ground-line anchor does NOT shift between frames (same approach as the pigeon-look-f* family) — measure and report any anchor drift.
  > 4. Resize to a sensible common shipping size (similar scale to the existing pigeon-f*.png / pigeon-look-f*.png sprites, ~300x450, but use your judgement based on the standing silhouette's actual proportions) and compress with pngquant.
  > 5. Save shipped files to public/assets/sprites/pigeon-perch-f0.png (away), pigeon-perch-f1.png (mid-turn), pigeon-perch-f2.png (camera-facing smirk).
  > 6. QA: composite each shipped cutout over dark slate #3a405a, report corner-pixel alphas and any near-magenta pixel counts, and report the body/feet bounding-box position for all three frames so I can confirm they're anchor-stable.
  > 7. Append a provenance entry to assets/ART_LOG.md: date, master and shipped paths, references viewed, exact prompt (this message), output dimensions, panel semantics, anchor-drift measurements, and every post-processing step actually performed.
  >
  > DELIVERABLE: reply with the absolute paths of the master and all three shipped files, their final pixel dimensions, and confirmation the ART_LOG.md entry was appended.
- **Accepted refinement prompt (verbatim):**
  > Revise the supplied current candidate assets/masters/pigeon-perch-sheet.png into a production-safe three-panel sprite sheet. Preserve its successful standing/perched concept, character identity, folded wings, palette, rendering, pure #ff00ff background, panel order, and overall expressions. Apply these precise corrections only:
  >
  > 1. Enforce three exact equal vertical panel cells across one wide sheet. Scale every pigeon uniformly down by about 15 percent and center it within its own third so each complete silhouette has at least 45 pixels of pure-magenta clearance from BOTH vertical panel boundaries and from every outer image edge. No pigeon may cross, overlap, or touch a neighboring panel boundary. Keep generous top clearance and feet above the bottom.
  > 2. Lock the drawing from the lower neck downward: the torso, chest, folded wing, rump, tail, legs, toes, body scale, body x/y, and foot baseline must be visually identical in all three panels. Duplicate that exact body drawing panel-to-panel. ONLY redraw the head and very upper neck orientation.
  > 3. Panel 1 stays calm profile looking away/down. Panel 2 stays a mid-turn toward viewer, but make the emerging orange eye half-lidded and sly rather than wide-open or surprised. Panel 3 stays dead-on with both eyes half-lidded, purple eye patches, crooked knowing beak grin, and one raised scruffy brow, matching pigeon-look-sheet.png.
  > 4. Keep wings completely folded/tucked, tail down, both feet planted gripping an invisible horizontal edge. No flying silhouette, spread wing, airborne pose, dangling feet, ledge, shadow, divider, skyline, object, text, border, or watermark.
  > 5. Background remains perfectly uniform solid pure #ff00ff with no variation. Nothing touches any image edge.
  >
  > Reference roles: the current candidate is the edit target/layout to refine; pigeon-look-sheet.png supplies the panel-3 conspiratorial face; normal.png and pleased.png supply identity/style; pigeon-flight-sheet.png supplies full-body palette and feather language only, never its flight pose.

## assets/masters/title-logo.png → public/assets/sprites/title-logo.png — 2026-07-20

- **Purpose/semantics:** Standalone, single-line title-screen logotype reading exactly `PIGEON DROP`; chunky comic lockup with gooey drips hanging from every letter (visual pun on the game's raunchy dropping gag). Ten unambiguous capital letters (`P-I-G-E-O-N`, word space, `D-R-O-P`); no scene, subtitle, border, watermark, pigeon, or extra icons.
- **Revision note:** Replaces the earlier 2026-07-20 clean golden-amber bubble-letter candidate (master was 1983×793 / shipped 1600×339). That pass read too family-casual for the game's absurdist tone; this revision leans into dripping cream letterforms.
- **References inspected:** `images/normal.png` (style-only: thick outlines, cel shading, raunchy comic personality — pigeon subject excluded), `public/assets/sprites/title-bg.png` (color/mood-only dusk sky for overlay contrast — scenery excluded).
- **Tool:** Cursor session `GenerateImage` (Composer image generation). One accepted render. Untouched generator output copied to `assets/masters/title-logo.png`; SHA-256 `ffd8fb130526ca0a9ec213e0d862f465cc7869f481c4b286e1212c14463e3a9c`.
- **Dimensions/hashes:** Untouched master: 1536×1024, opaque sRGB true-color PNG. Shipped: 1516×513, indexed sRGBA PNG with 256 colors, 351,374 bytes, SHA-256 `6d9f3455e1d5107643107fda48c91f13cafd7c1fa56984d2d9e355288539f294`.
- **Post-processing:** (1) preserved generator bytes at the master path; (2) keyed a working copy with `convert assets/masters/title-logo.png -fuzz 20% -transparent '#ff00ff' -channel A -morphology Erode Disk:1 +channel` (20% fuzz because the source chroma field is near-magenta, not exact `#ff00ff` — corner samples ≈ `(239,10,238)`–`(245,16,240)`); (3) `-trim +repage` then `-bordercolor none -border 28`; (4) `-resize '1600x>' -strip` (native trimmed width was already 1516, so no upscale); (5) `pngquant --force --output public/assets/sprites/title-logo.png 256` on the true-color working copy; (6) QA composites at `assets/masters/title-logo-qa-slate.png` (over `#3a405a`) and `assets/masters/title-logo-qa-bg.png` (upper half of `title-bg.png`, logo scaled like TitleScene at ~22% of H).
- **Runtime layout:** `TitleScene` scales the logo to `H * 0.22` (was 0.16) and places it at `H * 0.18`, with the tagline lowered to `H * 0.36` so the drips clear the supporting line.
- **QA:** Visually inspected master, shipped cutout, slate QA, and title-bg QA. Spelling reads unambiguously as `PIGEON DROP` in every view. Alpha content bbox `(28,28)-(1488,485)` with 28 px transparent clearance on each side. Corner alpha `[0,0,0,0]`. Near-magenta nonzero-alpha pixels (`R>=224, G<=40, B>=224`) = `0`. Cream drippy lockup remains legible over both slate and the bright sunset sky band.
- **Exact generation prompt:**
  > Game title logo wordmark for an adult absurdist cartoon game. Exact readable text in all caps: PIGEON DROP (two words, one line, correct spelling). Heavy cartoon logotype with thick clean dark outlines and painterly cel shading. Raunchy mischievous vibe: chunky irregular comic letters that look a bit crude and street-smart, not cute bubble-font casual mobile-game. Creamy off-white to dirty pale-yellow letter fills with subtle warm amber shading near the bottoms, as if lightly stained. Several letter bottoms drip and sag with thick gooey drips hanging down (suggesting bird droppings humor without drawing literal feces piles). Thin pale highlight along top edges of letters. Deep near-black outline with a slight dark brown extrusion/shadow underneath so it pops. Slight uneven kerning and a touch of mischievous tilt, still fully legible. No pigeon, bird, character, city, sun, clouds, scenery, icons, sparkles, badges, or extra decoration besides the dripping wordmark itself. Entire background must be solid flat pure magenta #ff00ff with no gradients, texture, shadows, or vignette. Wordmark centered with generous clear margin from all edges; nothing touches the frame edge. Landscape wide composition suitable as a transparent game overlay, roughly 2.5:1 aspect.
- **Kept as canonical 2026-07-20.** Briefly reconsidered as too heavily "shit-oriented" (drips
  hanging from every letter) and a byte-identical backup was made at
  `assets/masters/title-logo-raunchy-variant.png` /
  `-shipped.png` while a cleaner alternative (single restrained drip accent instead of dripping
  from every letter) was generated. On review the user decided this original dripping version
  actually fits the game's raunchy tone better and asked to keep it — it remains the shipped
  `assets/masters/title-logo.png` / `public/assets/sprites/title-logo.png`. The alternative
  single-drip pass was kept for reference, not shipped, at
  `assets/masters/title-logo-balanced-variant.png` /
  `assets/masters/title-logo-balanced-variant-shipped.png` (1600×401; clean bubble letters, one
  drip off the final "P"; same references and prompting approach as this entry, minus the
...
## 2026-07-21 — OpenAI Build Week project thumbnail

- **Generation tool:** built-in `image_gen` tool; underlying model not exposed by the tool.
- **Master:** `assets/masters/pigeon-drop-openai-build-week-thumbnail-master.png` — untouched generated 1536×1024 PNG.
- **Shipped promo asset:** `assets/promo/pigeon-drop-openai-build-week-thumbnail.png` — final 1536×1024 PNG.
- **References inspected and supplied:** `images/normal.png` (pigeon identity and expression), `public/assets/sprites/title-bg.png` (sunset rooftop setting and palette), `assets/masters/pigeon-flight-sheet.png` (full-body flight anatomy and rendering), `assets/masters/pedestrians-react-sheet.png` (angry pedestrian energy and rendering). `public/assets/sprites/title-logo.png` was inspected separately and composited unchanged after generation.
- **Composition semantics:** single 3:2 promotional key-art frame; large flying pigeon in the right-center foreground, one falling cream dropping, three reacting adults, one battered red compact car, one hydrant, dense sunset city backdrop, upper-left typography zone.
- **Post-processing:** copied the generated PNG unchanged to the master path; drew a rounded dark `OPENAI BUILD WEEK` badge with cream border/text using ImageMagick and DejaVu Sans Bold; resized the existing transparent `title-logo.png` to 660 px wide and composited it at `(45,48)`; retained the original 1536×1024 canvas; no crop, generative edit, color correction, or compression pass.
- **Visual QA:** inspected the generated master at full resolution and the final composition at 750×500 thumbnail scale. Accepted for correct pigeon identity, readable silhouette, clear gameplay gag, exact title art, crisp event text, safe margins, and correct 3:2 dimensions.

### Exact final generation prompt

> Use case: ads-marketing
> Asset type: 3:2 OpenAI Build Week project thumbnail, final background/key art to receive exact typography in post-production.
> References and role: Inspect all four references before generating. Image 1 is the immutable Pigeon Drop character identity and facial-expression reference. Image 2 is the established warm sunset rooftop city atmosphere and palette. Image 3 defines the slate-blue full-body pigeon anatomy, wing shapes, black crest, thick clean outlines, painterly cel shading, and iridescent green-to-purple neck feathers; use it as identity/style reference, not as a sprite sheet layout. Image 4 defines the game's deliberately exaggerated angry-pedestrian energy and human rendering style only.
> Primary request: Create a cinematic, funny, polished 3:2 key-art illustration for the game Pigeon Drop. A large slate-blue pigeon sweeps through the foreground from upper right toward center-left, seen in a dynamic three-quarter flight pose, turning its head toward the viewer with the exact conspiratorial smug wink and crooked grin from Image 1. Beneath it, a single comedic cream-colored pigeon dropping falls toward a city street where three tiny exaggerated adults react in outrage beside one battered red compact car. The bird feels gleefully guilty; the humans feel comically doomed, not harmed. Include one red fire hydrant at street level as a subtle gameplay callback.
> Scene/backdrop: Warm orange-and-purple sunset over a dense worn brick city, rooftop silhouettes and water towers receding into atmospheric depth, with the street visible in the lower quarter. Keep the mood lively and mischievous.
> Composition/framing: Exact 3:2 landscape, intended final size 1536x1024. Strong readable thumbnail silhouette. The pigeon occupies the right-center and is the unmistakable focal point, but do not crop wings, beak, or tail. Keep the upper-left quadrant and a narrow upper-right corner relatively calm and uncluttered for a wordmark and small event badge to be added later. Use a sweeping diagonal from bird to falling drop to reacting street characters. Preserve generous safe margins around the frame.
> Palette and rendering: Match Pigeon Drop exactly: polished 2D cartoon illustration, thick clean near-black outlines, painterly cel shading, warm upper-left sunset light, slate-blue pigeon, vivid orange eyes, purple eye patches, scruffy black brows/crest, iridescent green-to-purple neck feathers, warm brick reds, absurdist comic tone. Rich contrast but no neon overload.
> Invariants: The pigeon must remain the same recognizable character, not a realistic pigeon and not cute/innocent. Exactly one pigeon, three adult pedestrians, one compact car, and one hydrant. The pigeon dropping is opaque cream and small enough to remain humorous, not graphic.
> Text: none.
> Avoid: no words, letters, logos, badges, UI, watermark, borders, frames, sprite-sheet panels, magenta chroma background, blood, injury, gore, photorealism, 3D rendering, extra pigeons, extra limbs, cropped focal subject, clutter in the upper-left title area.
